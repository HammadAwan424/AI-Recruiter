...
export default function OfferManagementTab() {
  ...
  const fetchData = async () => {
    ...
    try {
      // Fetch offers
      const resOffers = await fetch("http://localhost:8000/offers", {
      ...
      // Fetch templates
      const resTemplates = await fetch("http://localhost:8000/offers/templates", {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (resTemplates.ok) {
      ...
      // Fetch candidates & jobs for dropdowns
      const resCandidates = await fetch("http://localhost:8000/recruitment/candidates", {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (resCandidates.ok) {
      ...
      const resJobs = await fetch("http://localhost:8000/recruitment/jobs", {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (resJobs.ok) {
  ...
  // Template change handler -> populates offer_letter_text
  const handleTemplateChange = (templateId) => {
    const selectedCand = candidates.find((c) => String(c.id) === String(formData.candidate_id));
    const selectedJb = jobs.find((j) => String(j.id) === String(formData.job_id));
    ...
    if (content) {
      content = content
        .replace(/{{candidate_name}}/g, selectedCand ? selectedCand.full_name : "[Candidate Name]")
        .replace(/{{job_title}}/g, formData.job_title || (selectedJb ? selectedJb.title : "[Job Title]"))
  ...
  const handleCreateOffer = async (submitDirectly = false) => {
    if (!formData.candidate_id || !formData.job_id || !formData.base_salary || !formData.start_date) {
      alert("Please fill in all required candidate, job, salary, and start date fields.");
      return;
    }

    try {
      const payload = {
        application_id: Number(formData.application_id) || Number(formData.candidate_id),
        candidate_id: Number(formData.candidate_id),
        job_id: Number(formData.job_id),
        job_title: formData.job_title || "Position",
        department: formData.department || "Engineering",
        base_salary: parseFloat(formData.base_salary),
        bonus_equity: formData.bonus_equity || "",
        start_date: formData.start_date,
        expiry_date: formData.expiry_date || formData.start_date,
        offer_letter_text: formData.offer_letter_text,
      ...
      if (res.ok) {
        ...
        alert(`Error: ${err.detail || "Failed to create offer"}`);
      }
    } catch (err) {
  ...
  const handleSubmitApproval = async (offerId) => {
    try {
      const res = await fetch(`http://localhost:8000/offers/${offerId}/submit-approval`, {
        method: "POST",
        headers: { Authorization: `Bearer ${token}` },
      });
      if (res.ok) {
  ...
  const handleApprovalAction = async (offerId, action) => {
    try {
      const res = await fetch(`http://localhost:8000/offers/${offerId}/approval`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
  ...
  const handleSendOffer = async (offerId) => {
    try {
      const res = await fetch(`http://localhost:8000/offers/${offerId}/send`, {
        method: "POST",
        headers: { Authorization: `Bearer ${token}` },
      });
      if (res.ok) {
        const updated = await res.json();
        const candidateLink = `http://localhost:5173/offer/sign/${updated.secure_token}`;
  ...
  const handleCopyLink = (secureToken) => {
  ...
  const filteredOffers = offers.filter((o) => {
    ...
    if (activeFilter === "PENDING_APPROVAL") return o.status === "PENDING_APPROVAL";
    if (activeFilter === "APPROVED") return o.status === "APPROVED";
    if (activeFilter === "SENT") return o.status === "SENT";
    if (activeFilter === "SIGNED") return o.status === "SIGNED";
    if (activeFilter === "DECLINED") return o.status === "DECLINED";
    if (activeFilter === "DRAFT") return o.status === "DRAFT";
    return true;
  });
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm overflow-y-auto">
          <div className="bg-[#111827] border border-[#05DC7F]/30 rounded-2xl p-6 w-full max-w-3xl my-8 text-white space-y-4 shadow-2xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="text-xs text-white/60 block mb-1">Select Candidate *</label>
                <select
