...
function SlotBuilderModal({ onClose, onCreated }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch("http://127.0.0.1:8000/interviews/slots", {
        method: "POST",
        headers: {
      ...
      <div className="w-full max-w-md rounded-2xl bg-black/90 border border-[#05DC7F]/40 p-6 shadow-[0_0_25px_rgba(5,220,127,0.4)] relative">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <button
            type="submit"
            disabled={loading}
            className="mt-2 w-full py-3 rounded-xl font-semibold bg-[#05DC7F] text-black hover:bg-[#04c56f] transition"
          >
            {loading ? "Creating..." : "Save Availability Slot"}
          </button>
        </form>
      </div>
    </div>
  ...
  const copySelfScheduleLink = async () => {
    ...
    try {
      const res = await fetch(
        `http://127.0.0.1:8000/interviews/${data.id}/self-schedule-link`,
        {
          method: "POST",
      ...
      const link = `http://localhost:5173/interview/schedule/${updated.self_schedule_token}`;
      ...
      <div className="w-full max-w-lg rounded-2xl bg-black/90 border border-[#05DC7F]/40 p-6 sm:p-8 shadow-[0_0_25px_rgba(5,220,127,0.4)] relative">
        ...
        <div className="flex flex-col gap-3 text-gray-300 text-sm sm:text-base">
          <p className="flex items-center gap-2">
            ...
            <span className="text-white font-semibold">{data.candidate_name}</span>
          </p>
          <p className="flex items-center gap-2">
...
export default function InterviewsTab() {
  ...
  const [viewMode, setViewMode] = useState("Agenda"); // Agenda | Month | Slots
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const resInt = await fetch("http://127.0.0.1:8000/interviews", {
        headers: { Authorization: `Bearer ${token}` },
      });
      const dataInt = await resInt.json();
      setInterviews(Array.isArray(dataInt) ? dataInt : []);

      const resSlots = await fetch("http://127.0.0.1:8000/interviews/slots", {
        headers: { Authorization: `Bearer ${token}` },
      });
              ...
              >
                <div className="flex flex-col gap-1">
                  ...
                  <div className="flex flex-wrap gap-4 text-gray-400 text-xs mt-2">
                    ...
                    <span className="flex items-center gap-1.5">
                      <FaClock className="text-[#05DC7F]" /> {item.scheduled_time} ({item.duration_minutes}m)
                    </span>
                  </div>
                </div>
