...
export const DashboardOverviewPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        const res = await fetch("http://127.0.0.1:8000/ceo/dashboard-stats", {
  ...
  const pipelineData = [
    { name: "Applied", value: stats.pipeline?.applied || 0, title: "Applied Candidates", unit: "" },
    { name: "Shortlisted", value: stats.pipeline?.shortlisted || 0, title: "Shortlisted Candidates", unit: "" },
    { name: "Interviews", value: stats.pipeline?.interviews || 0, title: "Interviews Scheduled", unit: "" },
    { name: "Hired", value: stats.pipeline?.hired || 0, title: "Hired Candidates", unit: "" },
  ];
  ...
  return (
    <div className="flex flex-col gap-10 p-4 sm:p-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatCard title="Hired Employees" value={stats.total_employees} icon={<FaUsers className="text-[#05DC7F]" />} />
        <StatCard title="Departments" value={stats.total_departments} icon={<FaCalendarCheck className="text-[#05DC7F]" />} />
        <StatCard title="Active Openings" value={stats.active_openings} icon={<FaBriefcase className="text-[#05DC7F]" />} />
        <StatCard title="Monthly Payroll" value="$505K" icon={<FaDollarSign className="text-[#05DC7F]" />} />
      </div>
          ...
          <div className="h-64 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={attendanceData}>
                ...
                <Line type="monotone" dataKey="value" stroke="#05DC7F" strokeWidth={3} dot={{ fill: "#05DC7F" }} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </ChartBox>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="flex flex-col items-center gap-6">
            <div className="h-56 w-full">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Tooltip content={<CustomTooltip />} />
                  <Pie data={pipelineData} dataKey="value" innerRadius={60} outerRadius={90}>
        ...
        <ChartBox title="Recent Activity">
          <div className="flex flex-col gap-4">
            <Activity icon={<FaCheckCircle className="text-[#05DC7F]" />} title="New Job Requisition Posted" time="2 hours ago" />
            <Activity icon={<FaArrowUp className="text-blue-400" />} title="Candidate Interview Scheduled" time="4 hours ago" />
            <Activity icon={<FaClock className="text-yellow-400" />} title="Offer Letter Pending Approval" time="6 hours ago" />
          </div>
        </ChartBox>
      </div>
    </div>
