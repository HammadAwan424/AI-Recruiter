...
export const JobManagementPage: React.FC = () => {
  const [activeTab, setActiveTab] = useState<"Create Job" | "Job Posts">("Create Job");
  const { jobs, isLoading } = useJobs();
  const { createJob, deleteJob } = useJobMutations();

  const tabs = [
    { label: "Create Job" },
    { label: `Job Posts (${jobs.length})`, value: "Job Posts" },
  ];

  return (
    <div className="relative flex flex-col gap-6 sm:gap-8 p-4 sm:p-6">
      {/* ===== LEGENDARY PILL HEADER TABS ===== */}
      ...
      {/* ===== TAB CONTENT ===== */}
      <GlassContainerCard>
        {activeTab === "Create Job" && <CreateJobForm onSubmit={createJob} />}
        {activeTab === "Job Posts" && (
